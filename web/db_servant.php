...
abstract class Servant {
    ...
    protected $host; // MYSQL Hostname 
    protected $user; // MYSQL Username 
    protected $pass; // MYSQL Password 
    protected $db;   // MYSQL Database 
   ...
   public function error($type=''){ 
	...
	/* CONVERT
	     ...
	     */
		private function theAdvent($table = NULL){
		    self::connect(); //connect to db
		    if ($this->range) {
		   ...
		   $stmt = $this->mysqli->stmt_init();
        	...
        	print "<article><h5>{$this->getBook()} {$this->getChapter()}</h5>";
        	/* fetch values */
            while ($stmt->fetch()){ 
               // printf("%s %s\n", $v, $t); 
                print "<div class=\"versetext\"><div class=\"versenum\">$v</div> ${t}</div><br />";
	        }
	        print "</article>";
	...
	public function prepare($string, $range = NULL){
		...
		/* Translation
		 * Translation table marked for theAdvent
		 */
